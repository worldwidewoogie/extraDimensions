...
public class ChunkProviderMultiBiome implements IChunkProvider {
	private Random rand;
	...
	public ChunkProviderMultiBiome(World par1World, long par2, boolean par4) {
		...
		NoiseGenerator[] noiseGens = { this.noiseGen1, this.noiseGen2,
				this.noiseGen3, this.noiseGenPerl, this.noiseGen5,
		...
		this.noiseGenPerl = ((NoiseGeneratorPerlin) noiseGens[3]);
		this.noiseGen5 = ((NoiseGeneratorOctaves) noiseGens[4]);
		this.noiseGen6 = ((NoiseGeneratorOctaves) noiseGens[5]);
		this.mobSpawnerNoise = ((NoiseGeneratorOctaves) noiseGens[6]);

		this.caveGenerator = TerrainGen.getModdedMapGen(this.caveGenerator,
		...
		this.villageGenerator = ((MapGenVillage) TerrainGen.getModdedMapGen(
				this.villageGenerator, InitMapGenEvent.EventType.VILLAGE));
		this.mineshaftGenerator = ((MapGenMineshaft) TerrainGen
				.getModdedMapGen(this.mineshaftGenerator,
		...
		this.ravineGenerator = TerrainGen.getModdedMapGen(this.ravineGenerator,
	...
	public ChunkPosition func_147416_a(World world, String arg1, int arg2,
			int arg3, int arg4) {
		return null;
	}
	...
	private void func_147423_a(int p_147423_1_, int p_147423_2_, int p_147423_3_) {
		...
		this.noise5 = this.noiseGen6.generateNoiseOctaves(this.noise5,
		...
		for (int j1 = 0; j1 < 5; j1++) {
			for (int k1 = 0; k1 < 5; k1++) {
				...
				double d13 = this.noise5[i1] / 8000.0D;
					...
					for (int l2 = 0; l2 < 8; l2++) {
						...
						for (int i3 = 0; i3 < 4; i3++) {
							...
							for (int k3 = 0; k3 < 4; k3++) {
								if ((d15 += d16) > 0.0D) {
									blocks[j3 += short1] = Blocks.stone;// these
																		// can
																		// be
																		// set
																		// to
									...
									blocks[j3 += short1] = Blocks.water;// these
																		// can
																		// be
																		// set
																		// to
									...
									blocks[j3 += short1] = null;// this is the
																// air block i
																// think.
								}
							}
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		MinecraftForge.EVENT_BUS
				.post(new PopulateChunkEvent.Pre(par1IChunkProvider,
		...
		if ((biomegenbase != BiomeGenBase.desert)
				...
				&& (TerrainGen.populate(par1IChunkProvider, this.worldObj,
						this.rand, par2, par3, flag,
		...
		if ((TerrainGen.populate(par1IChunkProvider, this.worldObj, this.rand,
				par2, par3, flag, PopulateChunkEvent.Populate.EventType.LAVA))
		...
		boolean doGen = TerrainGen.populate(par1IChunkProvider, this.worldObj,
				this.rand, par2, par3, flag,
		...
		doGen = TerrainGen.populate(par1IChunkProvider, this.worldObj,
				this.rand, par2, par3, flag,
		...
		MinecraftForge.EVENT_BUS
				.post(new PopulateChunkEvent.Post(par1IChunkProvider,
						this.worldObj, this.rand, par2, par3, flag));
	...
	public Chunk provideChunk(int par1, int par2) {
		...
		this.biomesForGeneration = this.worldObj.getWorldChunkManager()
				.loadBlockGeneratorData(this.biomesForGeneration, par1 * 16,
						par2 * 16, 16, 16);
		replaceBlocksForBiome(par1, par2, ablock, abyte,
	...
	public void replaceBlocksForBiome(int p_147422_1_, int p_147422_2_,
		...
		ChunkProviderEvent.ReplaceBiomeBlocks event = new ChunkProviderEvent.ReplaceBiomeBlocks(
				this, p_147422_1_, p_147422_2_, p_147422_3_, p_147422_5_);
		MinecraftForge.EVENT_BUS.post(event);
		if (event.getResult() == Result.DENY)
			return;

		double d0 = 0.03125D;
		this.stoneNoise = this.noiseGenPerl.func_151599_a(this.stoneNoise,
				p_147422_1_ * 16, p_147422_2_ * 16, 16, 16, d0 * 2.0D,
				d0 * 2.0D, 1.0D);
